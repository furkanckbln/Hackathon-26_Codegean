...
def sales_assistant_chat(
    ...
    high_refund_listings = context.get("high_refund_listings", [])
    ...
    # İade istatistikleri metni
    def _refund_text(items):
        if not items:
            return "  İade oranı yüksek (%5+) ilan yok."
        lines = []
        for l in items:
            lines.append(
                f"  - {l['title'][:50]} → "
                f"%{l['refund_rate']} iade oranı "
                f"({l['refund_count']} iade / {l['total_count']} sipariş)"
            )
        return "\n".join(lines)

    refund_text = _refund_text(high_refund_listings)
...
YÜKSEKİADE ORANLARI (%5 ve üzeri):
{refund_text}

Not: Sektör standardı iade oranı %3-5 altında olmalı. %10 ve üzeri kritik seviyedir.
İade sebepleri genellikle: yanlış ürün açıklaması, kargo hasarı, ürün kalitesi, boyut/renk uyumsuzluğu.
...
İade oranı yüksek ürünler sorulduğunda: hangi ilanların sorunlu olduğunu rakamlarla göster,
  olası iade sebeplerini analiz et (açıklama tutarsızlığı, kalite, kargo), somut iyileştirme adımları öner.
